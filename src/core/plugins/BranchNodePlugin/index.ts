...
import { type Node, type Edge, type ExecutionHistory } from "../../type.d";
...
export class BranchNodePlugin extends BaseNodePlugin {
  ...
  async getNextNodeId(edges: Edge[], pluginExecutionEngine: PluginExecutionEngine, historyItem?: ExecutionHistory): Promise<string | null> {
    ...
    console.log('[BranchNodePlugin] 可用的边:', edges.map(e => ({ id: e.id, target: e.target, conditions: e.conditions, isDefault: e.isDefault })));
    
    // 评估所有非默认边的条件
    for (const edge of edges) {
      if (!edge.isDefault) {
        try {
          console.log(`[BranchNodePlugin] 评估边 ${edge.id} 的条件:`, edge.conditions);
          const isMatch = await pluginExecutionEngine.evaluateRule(edge?.conditions, edge.id);
    ...
    if (matchedEdges.length > 1) {
      ...
      // 优先级倒序排序
      matchedEdges.sort((a, b) => (b.priority || 0) - (a.priority || 0));
    ...
    if(historyItem){
      historyItem.decision={
        ...
        conditions: matchedEdge.conditions,
        isDefault: !!matchedEdge?.isDefault 
      }
    }
