...
export class ParallelNodePlugin extends BaseNodePlugin {
  ...
  async executeNode(node: Node, pluginExecutionEngine: PluginExecutionEngine, historyItem?: ExecutionHistory): Promise<boolean> {
    ...
    // 如果没有设置并行策略，默认为ALL
    if (!node.config.parallel_strategy) {
      node.config.parallel_strategy = ParallelStrategy.ALL;
    }
    
    // 记录并行策略到历史记录
    if (historyItem) {
      historyItem.parallel_strategy = node.config.parallel_strategy;
    }
  ...
  async getNextNodeId(
    ...
    for (const edge of edges) {
      try {
        const isMatch = await pluginExecutionEngine.evaluateRule(edge?.conditions);
        if (isMatch || edge.isDefault) {
