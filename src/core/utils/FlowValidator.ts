...
import { type FlowDefinition, type Node, } from '../type.d';
...
export class FlowValidator {
  ...
  private validateNodeConfig(
    ...
    // 验证迭代节点配置
    if (node.type === "Trigger") {
      const iterationCount = node.config.iteration_count;
    ...
    if (node.type === "Parallel") {
      const strategy = node.config.parallel_strategy;
   ...
   */
  public validateAndThrow(flow: FlowDefinition): void {
