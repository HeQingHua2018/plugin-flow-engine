...
import  { type Edge, type Node, type ExecutionHistory } from '../type.d';
import type { NodePlugin } from '../plugins/NodePlugin.d';
import { PluginExecutionEngine } from './PluginExecutionEngine';
import { getBuiltInPluginInstances } from '../plugins/index'
// 使用注入器注册 schema 与自定义组件
import { initializeInjector, injectNodeSchema, injectWidget, getNodeSchemas } from '../utils/NodeInjector';
 ...
 */
  ...
  private registerBuiltInPlugins(): void {
    ...
    console.log(this.getAllNodeTypes());
    console.log('[PluginManager] 已自动注册所有内置节点插件并完成配置注入（仅 schema 与组件）');
  }
 ...
 */
  public registerPlugin(plugin: NodePlugin): void {
    ...
    console.log(`[PluginManager] 已注册节点类型 ${plugin.nodeType} 的插件`);
    // 同步注入该插件对应的 schema、控件（节点类型已存在，不再注入）
    try {
      const cfg = plugin.getNodeFormConfig?.();
      if (cfg?.schema) {
        injectNodeSchema(cfg.schema.type, cfg.schema);
      }
      if (cfg?.widgets) {
        Object.entries(cfg.widgets).forEach(([key, widget]) => {
          injectWidget(key, widget as any);
        });
      }
    } catch (e) {
      console.warn('[PluginManager] 节点配置注入失败:', e);
    }
  }
...
/**
 * 获取指定类型的节点插件
 * @param nodeType 节点类型枚举值
 * @returns 对应的节点插件实例
 * @throws 当找不到对应类型的插件时抛出错误
 */
  public getPlugin(nodeType: string): NodePlugin {
    // 不设置默认节点
    const plugin = this.plugins.get(nodeType);
    if (!plugin) {
      throw new Error(`未找到节点类型 ${nodeType} 的插件`);
    }
    return plugin;
  }
   ...
   * @returns 是否存在插件
   */
  public hasPlugin(nodeType: string): boolean {
  ...
  public getNodeFormConfig(nodeType: string): any {
    ...
    try {
      ...
      injected = schemas[nodeType];
    } catch (e) {
      // 忽略注入器未初始化的情况，继续回退
    }
    const plugin = this.getPlugin(nodeType);
  ...
  /**
   * 获取所有节点类型列表
   * @returns 节点类型列表，格式为[{value: nodeType, label: nodeTypeName}]
   */
  public getAllNodeTypes(): Array<{value: string, label: string}> {
    return this.getAllPlugins().map(plugin => ({
      value: plugin.nodeType,
      label: plugin.nodeTypeName
    }));
  }
   ...
   * @returns 返回节点执行状态
   */
  public async getExecuteNodeStatus(node: Node, pluginExecutionEngine: PluginExecutionEngine): Promise<string | string[] | null> {
