 ...
 */
   ...
   * @param context 执行上下文
   */
  initialize(flow: FlowDefinition, context: ExecutionContext) {
    // 验证流程定义
    try {
      flowValidator.validateAndThrow(flow);
    } catch (error) {
  ...
  public async getNextNodeId(
    ...
    const plugin = this.pluginManager.getPlugin(node.type);
    if (plugin) {
   ...
   * @returns 节点状态或null
   */
  public async getNodeStatus(nodeId: string): Promise<string | null> {
  ...
  public async executeNode(nodeId: string): Promise<boolean> {
    ...
    // 如果没有对应类型的插件，无法执行
    if (!plugin) {
      const flowError = FlowExecutionError.pluginNotFound(node.type);
      throw errorHandler.handleError(flowError);
    }
    ...
    try {
      ...
      if (node.config.event) {
        historyItem.event = node.config.event;
      }
      ...
      if (historyItem.startTime && historyItem.endTime) {
        const executionTime = historyItem.endTime.getTime() - historyItem.startTime.getTime();
        flowMonitor.recordNodeExecution(node.id, executionTime, historyItem.status);
      }
  ...
  private checkForFailedNodes(): boolean {
    return this.executionHistory.some((item) => item.status === "failed");
  }
