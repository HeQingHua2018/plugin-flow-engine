...
import { globalRuleEngineCache } from './RuleEngineCache';
...
export interface NodeStatistics {
  nodeId: string;
  nodeName: string;
  pluginNodeType: PluginNodeType | 'string';
...
export class FlowMonitor {
  ...
  private getCurrentMemoryUsage(): number {
    // 浏览器环境：使用 performance.memory API（Chrome/Edge）
    ...
    // Node.js 环境：使用 process.memoryUsage()
    if (typeof process !== 'undefined' && process.memoryUsage) {
      return process.memoryUsage().heapUsed;
    }

    // 降级方案：返回 0（无法获取精确内存信息）
    return 0;
  }
