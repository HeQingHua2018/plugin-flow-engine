...
export enum FlowErrorCode {
  ...
  CONTEXT_SNAPSHOT_NOT_FOUND = 'CONTEXT_SNAPSHOT_NOT_FOUND',

  // 事务相关错误
  TRANSACTION_NOT_ACTIVE = 'TRANSACTION_NOT_ACTIVE',
  TRANSACTION_NOT_FOUND = 'TRANSACTION_NOT_FOUND',
  TRANSACTION_NESTED_LIMIT_EXCEEDED = 'TRANSACTION_NESTED_LIMIT_EXCEEDED',
...
export class FlowExecutionError extends Error {
   ...
   * 创建插件未找到错误
   */
  static pluginNotFound(pluginNodeType: string): FlowExecutionError {
    return new FlowExecutionError(
   ...
   * 创建上下文管理器未初始化错误
   */
  static contextManagerNotInitialized(message: string): FlowExecutionError {
    return new FlowExecutionError(
      message,
      FlowErrorCode.CONTEXT_MANAGER_NOT_INITIALIZED,
      ErrorSeverity.HIGH
    );
  }
   ...
   * 创建上下文变量未找到错误
   */
  static contextVariableNotFound(message: string): FlowExecutionError {
    return new FlowExecutionError(
      message,
      FlowErrorCode.CONTEXT_VARIABLE_NOT_FOUND,
      ErrorSeverity.MEDIUM
    );
  }
   ...
   * 创建上下文快照未找到错误
   */
  static contextSnapshotNotFound(message: string): FlowExecutionError {
    return new FlowExecutionError(
      message,
      FlowErrorCode.CONTEXT_SNAPSHOT_NOT_FOUND,
      ErrorSeverity.MEDIUM
    );
  }
   ...
   * 创建事务未激活错误
   */
  static transactionNotActive(message: string): FlowExecutionError {
    return new FlowExecutionError(
      message,
      FlowErrorCode.TRANSACTION_NOT_ACTIVE,
      ErrorSeverity.HIGH
    );
  }
   ...
   * 创建事务未找到错误
   */
  static transactionNotFound(message: string): FlowExecutionError {
    return new FlowExecutionError(
      message,
      FlowErrorCode.TRANSACTION_NOT_FOUND,
      ErrorSeverity.MEDIUM
    );
  }
   ...
   * 创建事务嵌套层级超限错误
   */
  static transactionNestedLimitExceeded(maxDepth: number): FlowExecutionError {
    return new FlowExecutionError(
      `事务嵌套层级超过限制 (${maxDepth})`,
      FlowErrorCode.TRANSACTION_NESTED_LIMIT_EXCEEDED,
      ErrorSeverity.HIGH
    );
  }
  ...
  getUserFriendlyMessage(): string {
    switch (this.code) {
      ...
      case FlowErrorCode.FLOW_TIMEOUT:
        return '操作超时，请稍后重试';
      case FlowErrorCode.TRANSACTION_NOT_ACTIVE:
        return '当前没有活跃的事务';
      case FlowErrorCode.TRANSACTION_NOT_FOUND:
        return '事务不存在或已提交/回滚';
      case FlowErrorCode.TRANSACTION_NESTED_LIMIT_EXCEEDED:
        return '事务嵌套层级过深，请简化操作';
      default:
        return '系统错误，请联系技术支持';
    }
  }
