 ...
 */
import type { Edge, Node, PluginNodeType } from '../types';
...
export class FlowGraphOps {
  ...
  insertNodeOnEdge = (edgeId: string, pluginNodeType: PluginNodeType): void => {
    ...
    const newEdgeA: Edge = { id: `e_${Date.now()}_a`, type: 'basic_edge', source: edge.source, target: newNodeId, data: (edge as any)?.data } as any;
