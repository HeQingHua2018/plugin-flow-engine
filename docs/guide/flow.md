...
- 核心类型请参见：[FlowDefinition](./types#flowdefinition)、[Node](./types#node)、[Edge](./types#edge)、[ExecutionContext](./types#executioncontext)、[ExecutionHistory](./types#executionhistory)、[GlobalConfig](./types#globalconfig)。
- 条件判断依赖 `json-rules-engine`，并支持扩展的自定义操作符，详见 [规则操作符](./operators)。
- 分支优先级与默认分支请使用 `priority` 与 `isDefault`；并行与迭代策略参见 [ParallelStrategy](./types#parallelstrategy) 与 [IterationMode](./types#iterationmode)。
...
- 并行/迭代：根据业务选择 `[ParallelStrategy](./types#parallelstrategy)` 与 `[IterationMode](./types#iterationmode)`。
...
- 类型速查与完整字段说明：[./types](./types)
- 规则操作符与扩展用法：[./operators](./operators)
- 自定义控件与表单 Schema：[./widgets](./widgets)
- 插件开发规范与替换策略：[./plugins](./plugins)
