...
- 快速上手（本页）：安装与最小可用示例
- 插件规范：如何开发与注册节点插件（[查看详情](./plugins.md)）
- 自定义控件：如何为节点表单扩展控件（[查看详情](./widgets.md)）
- 类型速查：统一查看所有类型定义（[查看详情](./types.md)）
- 流程数据定义：核心类型与配置示例（[查看详情](./flow.md)）
- 规则操作符：扩展 json-rules-engine 自定义操作符（[查看详情](./operators.md)）
- 组件与示例：更多完整演示与 UI 说明（[查看示例](./components.md)）
...
- 更多完整演示与 UI 集成方式，[查看示例](./components.md)。
- 复杂流程（分支/并行/迭代）建模可参考[流程数据定义](./flow.md)。
